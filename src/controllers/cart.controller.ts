...
export const removeFromCart = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log('Remove cart item - ID:', id, 'Parsed:', itemId, 'UserID:', req.userId);

    if (isNaN(itemId)) {
      console.log('Invalid cart item ID');
      return res.status(400).json({ error: 'Invalid cart item ID' });
    }
    ...
    console.log('Cart item check:', checkItems);

    if (checkItems.length === 0) {
      console.log('Cart item does not exist in database');
      return res.status(404).json({ error: 'Cart item not found' });
    }

    if (checkItems[0].userId !== req.userId) {
      console.log('Cart item belongs to different user:', checkItems[0].userId, 'vs', req.userId);
